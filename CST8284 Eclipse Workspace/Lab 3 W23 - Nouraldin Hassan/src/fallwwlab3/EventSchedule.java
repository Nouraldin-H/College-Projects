...
//Calender datetime = Calendar;
	 ...
	 */
	private static String regName = "unknown"; // is static to prevent errors with `EventSchedule(year, month, day)` on `this()`
	/**
	...
	public EventSchedule() {
		this(Calendar.YEAR); // this() is used for chaining to the next constructor. sets `Calendar.YEAR`
		...
		setname("Nouraldin".trim()); // sets name to "Nouraldin"
	...
	public EventSchedule(int year) {
		this(Calendar.YEAR, Calendar.MONTH); // in this case, year is set to 1 (upon testing). You would use `year` instead of `Calendar.YEAR`
		setmonth(time.getMonthValue()); // sets the month to the current month using the month value getter inside the setter.
		setday(time.getDayOfMonth()); // same as above but with day
		setname(regName.trim()); // sets name to the string variable, "unknown" in this case
	...
	public EventSchedule(int year, int month) {
		this(year, month, Calendar.DAY_OF_MONTH); // have this() declare `year`, `month`, and `day` for next chain and for user to display passed values of `year` and `month` when used (see Event3 in TestDemo.java)
	...
	public EventSchedule(int year, int month, int day) {
		this(year, month, Calendar.DAY_OF_MONTH, regName); // regName is static as to prevent errors within the `this()` call.
		setname("unknown".trim()); // removing this line changes all printed `regName` to display the name set from EventSchedule().
	 ...
	 * @param month The month for this EventSchedule.
	 * @param day The day for this EventSchedule.
	 * @param regName The name for this EventSchednouralule.
	 */
	public EventSchedule(int year, int month, int day, String regName) {
		...
		EventSchedule.regName = regName.trim(); // Was this.regName = ""; same as above but for regName. Accessed in a static way.
	...
	public void setname(String regName) {
		EventSchedule.regName = regName; // Was this.regName = regName;
	}
